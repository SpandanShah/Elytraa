...
class AdmissionCutoff(models.Model):
    ROUND_CHOICES = [(1, 'Round 1'), (2, 'Round 2'), (3, 'Round 3')]

    course = models.ForeignKey(
        Course, on_delete=models.CASCADE, related_name='cutoffs'
    )
    year = models.IntegerField(default=2025)
    round = models.IntegerField(
        choices=ROUND_CHOICES, default=1,
        help_text="ACPC admission round (1, 2, or 3)"
    )
    category = models.CharField(max_length=50)  # GEN, SC, ST, SEBC, TFWS, etc.
    board = models.CharField(max_length=50, blank=True, default='')  # GUJCET, JEE
    ...
    def __str__(self):
        return f"{self.course} - {self.category} R{self.round} ({self.year})"
