...
class AdmissionPredictor:
    ...
    def predict_colleges(self, student_rank, category=None, board=None, cource_preference=None, min_results=15):
        """
        ...
        # If no preference given, consider all courses
        if not cource_preference or 'all' in [c.lower() for c in cource_preference]:
            cource_preference = [col for col in related_keywords.keys()]

        # Optional filtering by preferred course
        results = []

        # Loop through each course preference separately
        for pref in cource_preference:
            pref_lower = pref.lower().strip()
            subset_df = filtered_df.copy()

            # Expand preferences with related terms
            expanded_keywords = related_keywords.get(pref_lower, [pref_lower])

            # Filter by course match
            subset_df = subset_df[
                subset_df['Course_name'].apply(lambda x: any(term in x for term in expanded_keywords))
            ]

            if subset_df.empty:
                continue

            # Filter eligible colleges by rank range
            eligible_df = subset_df[
                (student_rank >= subset_df['Opening'] - self.tolerance) &
                (student_rank <= subset_df['Closing'] + (self.tolerance * 10))
            ].copy()

            # If empty, take top 15 by closing rank for that course
            # if eligible_df.empty:
            #     eligible_df = subset_df.sort_values(by='Closing', ascending=True).head(min_results).copy()

            if len(eligible_df) < min_results:
                additional = subset_df.sort_values(by='Closing', ascending=True).head(min_results)
            ...
            # Sort by safety first, then closing rank
            chance_order = {"Safe": 2, "Possible": 1, "Stretch": 0}
            eligible_df['ChanceOrder'] = eligible_df['Chance'].map(chance_order)
            eligible_df = eligible_df.sort_values(by=['ChanceOrder', 'Closing']).head(min_results)
        ...
        if results:
            ...
            print("No matching colleges found for any preference.")
            return pd.DataFrame()
...
if __name__ == "__main__":
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        for course, group in eligible_universities.groupby('Preferred_Course'):
            sheet_name = course[:31]  # Excel sheet name limit
            group.to_excel(writer, sheet_name=sheet_name, index=False)
        eligible_universities.to_excel(writer, sheet_name='All_Results', index=False)
